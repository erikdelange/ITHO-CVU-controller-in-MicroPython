...
| 4 SCK	 | P37 |
| 5 MISO | P36 |
| 6 GD02 | P38 | General IO pin
| 7 GD00 | -   | Unused
| 8 SS	 | P34 | Chip select a.k.a slave select

The picture shows the actual setup using a breadboard. Note that I've made a carrier board for the CC1101 module as its pin pitch is smaller than the 1.27mm used on the breadboard. The nice thing about using a Wemos S2 Pico is that it can be connected to an usb-c cable for power. It also has a small 132x32 OLED screen which I'm not using here.
...
File *cc1101.py* contains the CC1101 driver. It is inspired by various versions written in C which can be found on GitHub. See the file header for the specific repositories I used. This version does nothing new, only difference is it is coded in MicroPython. The relevant aspects of your microcontrollers and hardware design must be recorded in *config.py*. The code in the driver itself is hardware and design agnostic. The code on GitHub assumes you are using the hardware as specified in the previous paragraph. By running just *cc1101.py* by itself the connection to the CC1101 module is tested. Output like this will appear:
...
For debug purposes the webserver can be stopped by calling *IP address of your microcontroller*:80/api/stop or by pressing the separate button on the Wemos board.
